...
Stage 方式で自動化できる。詳細は [docs/pipeline.md](pipeline.md) を参照。
...
4. Stage 順序制御（product-catalog + currency → cart → frontend）
...
```
[tkn pipeline start] → [Stage 1: product-catalog + currency (並列)]
