...
GitOps ワークフローにおける「マニフェスト変更の自動化」レイヤーに位置し、開発者は `tkn pipeline start` で新バージョンのタグを指定するだけで、依存関係を考慮した段階的リリース（Stage 方式）が実行される。
...
| Stage 順序制御による段階的リリース | 自動 Rollback（Git revert は手動） |
...
```mermaid
graph LR
    NS[notify-pipeline-start<br/>Pipeline 開始通知]
    subgraph "Stage 1 (並列)"
    ...
    NR[notify-pipeline-result<br/>Pipeline 完了通知<br/>finally task]
...
- `notify-pipeline-start`: Pipeline 開始時に Slack 通知（青）を送信
- Stage 1 の 2 タスクは `notify-pipeline-start` 完了後に並列実行される
- Stage 2 は Stage 1 の全タスク完了後、Stage 3 は Stage 2 完了後に開始する
- いずれかの Stage で失敗した場合、後続の Stage はスキップされる
- `notify-pipeline-result`: `finally` task として Pipeline の成否に関わらず実行される。成功時は緑、失敗時は赤の Slack 通知
...
- **git-push**: Stage 1 の並列タスクが同一ブランチに push するため、`rejected` エラーが発生しうる。失敗時は `git reset → pull → kustomize re-apply → commit → push` を最大 5 回リトライする。
...
| **全コンポーネント同一タグ** | Stage 内の全コンポーネントに同じ `image-tag` が適用される。個別タグ指定は未対応 |
